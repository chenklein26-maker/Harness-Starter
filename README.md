...
### 2026-06-23 — 瘦身 + Ponytail 融合

- **瘦身** — `npx` 只装 14 个 L2 核心文件，L3+ 可选
- **Ponytail 6 级梯子** — Simplicity First 升级为 YAGNI → 标准库 → 平台原生 → 已有依赖 → 一行 → 最少
- **自动格式化优化** — 先 `--check` 再 `--write`，仅问题文件触发
